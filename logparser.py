...
parser.add_argument('--to-file', nargs='?', default='access_report.json')
...
def load_data(files):
    ...
    try:
        df = pd.concat((pd.read_csv(f, delim_whitespace=True, names=cols.keys(),
                                    dtype=cols, keep_default_na=False, na_values=["-", ""]) for f in files)).drop(columns='l')
...
def prepare_report(df: pd.DataFrame):
    ...
    rep['top_hosts'] = df['host'].value_counts(dropna=True, ascending=False).head(3).to_dict()

    long_reqs = df.sort_values(by=['mils'], ascending=False).drop(columns=['user', 'status', 'bytes', 'referer', 'ua']).head(3)
    rep['top_long_requests'] = long_reqs.astype(object).replace(np.nan, 'None').to_dict(orient='records')
...
def main():
    ...
    rep = prepare_report(load_data(parse_args(parser)))
